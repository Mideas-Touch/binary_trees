...
A BST may be used to store data in non-contiguous locations is computer memory. Each node of the BST will have two pointers; to the left node and the right node. Additionally, each node of the BST typically will have a key, a value and two pointers for the left and right nodes. The leaves of the BST will have null left and right pointers a they do not show any other node.
